# Bitcoin Power Law Analysis and Retirement Planning

## Bitcoin Price (Linear Scale)
![Bitcoin Price Linear](images/btc_price_linear.png)

## Bitcoin Price (Log-Linear Scale)
![Bitcoin Price Log-Linear](images/btc_price_log_linear.png)

## Bitcoin Price (Log-Log Scale)
![Bitcoin Price Log-Log](images/btc_price_log_log.png)

## Log-Log Power Law Model
![Log-Log Power Law Model](images/log_log_history_only.png)

## Log-Log Power Law Model Future
![Log-Log Power Law Model](images/log_log.png)

## Linear-Log Scale Model
![Linear-Log Scale Model](images/linear_log.png)

## Limited Range Analysis
![Limited Range Analysis](images/limited_range.png)

## Linear-Linear Scale Model
![Linear-Linear Scale Model](images/linear_linear.png)

## Quantile Ratio Analysis
![Quantile Ratio Analysis](images/quantile_ratio.png)

## Years Ahead of Conservative Estimate
![Years Ahead Analysis](images/years_ahead.png)

## Raw ROI Analysis

Is the backwards looking ROI converging towards the ROI predicted by the power law?

![Raw ROI Analysis](images/roi_raw.gif)

## Dollar Cost Averaging Strategy
![DCA Strategy](images/dca_strategy_1.png)

# Appendix: convergence of power law over time

The lower bound (blue line) of the lower law stabilized around 2016 or 2017:

![](images/log_log.gif)
